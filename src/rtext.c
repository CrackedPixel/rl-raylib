...
*           Load default raylib font on initialization to be used by raylib_draw_text() and raylib_measuretext().
*           If no default font loaded, raylib_draw_text_ex() and raylib_measuretextex() are required.
*
...
*           On font atlas image generation [raylib_genimagefontatlas()], add a 3x3 pixels white rectangle
*           at the bottom-right corner of the atlas. It can be useful to for shapes drawing, to allow
*           drawing text and shapes with a single draw call [raylib_setshapestexture()].
*
...
#include "rlgl.h"           // OpenGL abstraction layer to OpenGL 1.1, 2.1, 3.3+ or ES2 -> Only raylib_drawtextpro()

#include <stdlib.h>         // Required for: malloc(), free()
#include <stdio.h>          // Required for: vsprintf()
#include <string.h>         // Required for: strcmp(), strstr(), strcpy(), strncpy() [Used in raylib_textreplace()], sscanf() [Used in LoadBMFont()]
#include <stdarg.h>         // Required for: va_list, va_start(), vsprintf(), va_end() [Used in raylib_textformat()]
#include <ctype.h>          // Required for: toupper(), tolower() [Used in raylib_texttoupper(), raylib_texttolower()]
...
//----------------------------------------------------------------------------------
    ...
    #define MAX_TEXT_BUFFER_LENGTH              1024        // Size of internal static buffers used on some functions:
                                                            // raylib_textformat(), raylib_textsubtext(), raylib_texttoupper(), raylib_texttolower(), raylib_texttopascal(), TextSplit()
#endif
...
// NOTE: Default font is loaded on raylib_initwindow() and disposed on raylib_close_window() [module: core]
static raylib_font defaultFont = { 0 };
#endif
...
static raylib_font LoadBMFont(const char *fileName);   // Load a BMFont file (AngelCode font file)
#endif
#if defined(SUPPORT_FILEFORMAT_BDF)
static raylib_glyphinfo *LoadFontDataBDF(const unsigned char *fileData, int dataSize, int *codepoints, int codepointCount, int *outFontSize);
#endif
...
extern void LoadFontDefault(void)
{
    ...
    // This way, we reconstruct raylib_font without creating large global variables
    ...
    defaultFont.texture = raylib_loadtexturefromimage(imFont);
    ...
    defaultFont.glyphs = (raylib_glyphinfo *)RL_MALLOC(defaultFont.glyphCount*sizeof(raylib_glyphinfo));
    defaultFont.recs = (raylib_rectangle *)RL_MALLOC(defaultFont.glyphCount*sizeof(raylib_rectangle));
    ...
    for (int i = 0; i < defaultFont.glyphCount; i++)
    {
        ...
        defaultFont.glyphs[i].image = raylib_imagefromimage(imFont, defaultFont.recs[i]);
    }

    raylib_unloadimage(imFont);
...
extern void UnloadFontDefault(void)
{
    for (int i = 0; i < defaultFont.glyphCount; i++) raylib_unloadimage(defaultFont.glyphs[i].image);
    raylib_unloadtexture(defaultFont.texture);
...
raylib_font raylib_getfontdefault()
{
#if defined(SUPPORT_DEFAULT_FONT)
    return defaultFont;
#else
...
// Load raylib_font from file into GPU memory (VRAM)
raylib_font raylib_loadfont(const char *fileName)
{
...
#if defined(SUPPORT_FILEFORMAT_TTF)
    if (raylib_isfileextension(fileName, ".ttf") || raylib_isfileextension(fileName, ".otf")) font = raylib_loadfontex(fileName, FONT_TTF_DEFAULT_SIZE, NULL, FONT_TTF_DEFAULT_NUMCHARS);
    else
#endif
#if defined(SUPPORT_FILEFORMAT_FNT)
    if (raylib_isfileextension(fileName, ".fnt")) font = LoadBMFont(fileName);
    else
#endif
#if defined(SUPPORT_FILEFORMAT_BDF)
    if (raylib_isfileextension(fileName, ".bdf")) font = raylib_loadfontex(fileName, FONT_TTF_DEFAULT_SIZE, NULL, FONT_TTF_DEFAULT_NUMCHARS);
    else
#endif
    {
        raylib_image image = raylib_load_image(fileName);
        if (image.data != NULL) font = raylib_loadfontfromimage(image, raylib_magenta, FONT_TTF_DEFAULT_FIRST_CHAR);
    ...
    {
        raylib_settexturefilter(font.texture, TEXTURE_FILTER_POINT);    // By default, we set point filter (the best performance)
...
// Load raylib_font from TTF or BDF font file with generation parameters
...
raylib_font raylib_loadfontex(const char *fileName, int fontSize, int *codepoints, int codepointCount)
{
    ...
    if (fileData != NULL)
    {
        ...
        font = raylib_loadfontfrommemory(raylib_getfileextension(fileName), fileData, dataSize, fontSize, codepoints, codepointCount);

        raylib_unloadfiledata(fileData);
    }
    else font = raylib_getfontdefault();

    return font;
}

// Load an raylib_image font file (XNA style)
raylib_font raylib_loadfontfromimage(raylib_image image, raylib_color key, int firstChar)
{
    ...
    raylib_font font = raylib_getfontdefault();
    ...
    font.texture = raylib_loadtexturefromimage(fontClear); // Convert processed image to OpenGL texture
    font.glyphCount = index;
    font.glyphPadding = 0;

    // We got tempCharValues and tempCharsRecs populated with chars data
    // Now we move temp data to sized charValues and charRecs arrays
    font.glyphs = (raylib_glyphinfo *)RL_MALLOC(font.glyphCount*sizeof(raylib_glyphinfo));
    ...
    for (int i = 0; i < font.glyphCount; i++)
    {
        ...
        // Fill character image data from fontClear data
        font.glyphs[i].image = raylib_imagefromimage(fontClear, tempCharRecs[i]);
    }

    raylib_unloadimage(fontClear);     // Unload processed image once converted to texture
...
raylib_font raylib_loadfontfrommemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount)
{
    ...
    strncpy(fileExtLower, raylib_texttolower(fileType), 16 - 1);
...
#if defined(SUPPORT_FILEFORMAT_TTF)
    if (raylib_textisequal(fileExtLower, ".ttf") ||
        raylib_textisequal(fileExtLower, ".otf"))
    {
        font.glyphs = raylib_loadfontdata(fileData, dataSize, font.baseSize, codepoints, font.glyphCount, FONT_DEFAULT);
    }
    else
#endif
#if defined(SUPPORT_FILEFORMAT_BDF)
    if (raylib_textisequal(fileExtLower, ".bdf"))
    {
    ...
    if (font.glyphs != NULL)
    {
        ...
        raylib_image atlas = raylib_genimagefontatlas(font.glyphs, &font.recs, font.glyphCount, font.baseSize, font.glyphPadding, 0);
        font.texture = raylib_loadtexturefromimage(atlas);

        // Update glyphs[i].image to use alpha, required to be used on raylib_imagedrawtext()
        for (int i = 0; i < font.glyphCount; i++)
        {
            raylib_unloadimage(font.glyphs[i].image);
            font.glyphs[i].image = raylib_imagefromimage(atlas, font.recs[i]);
        }

        raylib_unloadimage(atlas);
...
bool raylib_isfontready(raylib_font font)
{
...
raylib_glyphinfo *raylib_loadfontdata(const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount, int type)
{
    ...
    if (fileData != NULL)
    {
        ...
        if (stbtt_InitFont(&fontInfo, (unsigned char *)fileData, 0))     // Initialize font for data reading
        {
            ...
            chars = (raylib_glyphinfo *)RL_CALLOC(codepointCount, sizeof(raylib_glyphinfo));
...
//#define SUPPORT_FONT_ATLAS_SIZE_CONSERVATIVE
    ...
    // shapes and text can be backed into a single draw call: raylib_setshapestexture()
...
void raylib_unloadfontdata(raylib_glyphinfo *glyphs, int glyphCount)
{
    if (glyphs != NULL)
    {
        for (int i = 0; i < glyphCount; i++) raylib_unloadimage(glyphs[i].image);

        RL_FREE(glyphs);
    }
}

// Unload raylib_font from GPU memory (VRAM)
void raylib_unloadfont(raylib_font font)
{
    // NOTE: Make sure font is not default font (fallback)
    if (font.texture.id != raylib_getfontdefault().texture.id)
    {
        raylib_unloadfontdata(font.glyphs, font.glyphCount);
        raylib_unloadtexture(font.texture);
        RL_FREE(font.recs);
...
bool raylib_exportfontascode(raylib_font font, const char *fileName)
{
    bool success = false;
    ...
    strncpy(fileNamePascal, raylib_texttopascal(raylib_getfilenamewithoutext(fileName)), 256 - 1);
    ...
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    byteCount += sprintf(txtData + byteCount, "// FontAsCode exporter v1.0 - raylib_font data exported as an array of bytes                 //\n");
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    ...
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    byteCount += sprintf(txtData + byteCount, "// raylib_font name:    ....                                                                 //\n");
    byteCount += sprintf(txtData + byteCount, "// raylib_font creator: ....                                                                 //\n");
    byteCount += sprintf(txtData + byteCount, "// raylib_font LICENSE: ....                                                                 //\n");
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    byteCount += sprintf(txtData + byteCount, "////////////////////////////////////////////////////////////////////////////////////////\n\n");

    // Support font export and initialization
    // NOTE: This mechanism is highly coupled to raylib
    raylib_image image = raylib_loadimagefromtexture(font.texture);
    if (image.format != PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) TRACELOG(LOG_WARNING, "raylib_font export as code: raylib_font image format is not raylib_gray+ALPHA!");
    int imageDataSize = raylib_getpixeldatasize(image.width, image.height, image.format);

    // raylib_image data is usually GRAYSCALE + ALPHA and can be reduced to GRAYSCALE
    //raylib_imageformat(&image, PIXELFORMAT_UNCOMPRESSED_GRAYSCALE);

#define SUPPORT_COMPRESSED_FONT_ATLAS
#if defined(SUPPORT_COMPRESSED_FONT_ATLAS)
    // WARNING: Data is compressed using raylib raylib_compressdata() DEFLATE,
    // it requires to be decompressed with raylib raylib_decompressdata(), that requires
    // compiling raylib with SUPPORT_COMPRESSION_API config flag enabled

    // Compress font image data
    int compDataSize = 0;
    unsigned char *compData = raylib_compressdata((const unsigned char *)image.data, imageDataSize, &compDataSize);

    // Save font image data (compressed)
    byteCount += sprintf(txtData + byteCount, "#define COMPRESSED_DATA_SIZE_FONT_%s %i\n\n", raylib_texttoupper(fileNamePascal), compDataSize);
    byteCount += sprintf(txtData + byteCount, "// raylib_font image pixels data compressed (DEFLATE)\n");
    byteCount += sprintf(txtData + byteCount, "// NOTE: Original pixel data simplified to GRAYSCALE\n");
    byteCount += sprintf(txtData + byteCount, "static unsigned char fontData_%s[COMPRESSED_DATA_SIZE_FONT_%s] = { ", fileNamePascal, raylib_texttoupper(fileNamePascal));
    ...
    byteCount += sprintf(txtData + byteCount, "// raylib_font image pixels data\n");
    ...
    // Save font recs data
    byteCount += sprintf(txtData + byteCount, "// raylib_font characters rectangles data\n");
    ...
    byteCount += sprintf(txtData + byteCount, "// raylib_font glyphs info data\n");
    byteCount += sprintf(txtData + byteCount, "// NOTE: No glyphs.image data provided\n");
    byteCount += sprintf(txtData + byteCount, "static raylib_glyphinfo fontGlyphs_%s[%i] = {\n", fileNamePascal, font.glyphCount);
    for (int i = 0; i < font.glyphCount; i++)
    {
    ...
    byteCount += sprintf(txtData + byteCount, "// raylib_font loading function: %s\n", fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "static raylib_font LoadFont_%s(void)\n{\n", fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "    raylib_font font = { 0 };\n\n");
...
#if defined(SUPPORT_COMPRESSED_FONT_ATLAS)
    byteCount += sprintf(txtData + byteCount, "    // NOTE: Compressed font image data (DEFLATE), it requires raylib_decompressdata() function\n");
    byteCount += sprintf(txtData + byteCount, "    int fontDataSize_%s = 0;\n", fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "    unsigned char *data = raylib_decompressdata(fontData_%s, COMPRESSED_DATA_SIZE_FONT_%s, &fontDataSize_%s);\n", fileNamePascal, raylib_texttoupper(fileNamePascal), fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "    raylib_image imFont = { data, %i, %i, 1, %i };\n\n", image.width, image.height, image.format);
#else
    byteCount += sprintf(txtData + byteCount, "    raylib_image imFont = { fontImageData_%s, %i, %i, 1, %i };\n\n", styleName, image.width, image.height, image.format);
#endif
    byteCount += sprintf(txtData + byteCount, "    // Load texture from image\n");
    byteCount += sprintf(txtData + byteCount, "    font.texture = raylib_loadtexturefromimage(imFont);\n");
#if defined(SUPPORT_COMPRESSED_FONT_ATLAS)
    byteCount += sprintf(txtData + byteCount, "    raylib_unloadimage(imFont);  // Uncompressed data can be unloaded from memory\n\n");
#endif
    ...
    //  - 1. Data copy. This option consumes more memory and raylib_font MUST be unloaded by user, requiring additional code
    //  - 2. Data assignment. This option consumes less memory and raylib_font MUST NOT be unloaded by user because data is on protected DATA segment
//#define SUPPORT_FONT_DATA_COPY
    ...
    byteCount += sprintf(txtData + byteCount, "    // NOTE: Required to avoid issues if trying to free font\n");
    byteCount += sprintf(txtData + byteCount, "    font.glyphs = (raylib_glyphinfo *)malloc(font.glyphCount*sizeof(raylib_glyphinfo));\n");
    byteCount += sprintf(txtData + byteCount, "    memcpy(font.glyphs, fontGlyphs_%s, font.glyphCount*sizeof(raylib_glyphinfo));\n\n", fileNamePascal);
#else
    ...
    if (success != 0) TRACELOG(LOG_INFO, "FILEIO: [%s] raylib_font as code exported successfully", fileName);
...
{
    raylib_color color = raylib_lime;                         // Good FPS
    int fps = raylib_getfps();

    if ((fps < 30) && (fps >= 15)) color = raylib_orange;  // Warning FPS
    else if (fps < 15) color = raylib_red;             // Low FPS

    raylib_draw_text(raylib_textformat("%2i FPS", fps), posX, posY, 20, color);
}

// Draw text (using default font)
// NOTE: fontSize work like in any drawing program but if fontSize is lower than font-base-size, then font-base-size is used
// NOTE: chars spacing is proportional to fontSize
void raylib_draw_text(const char *text, int posX, int posY, int fontSize, raylib_color color)
{
    // Check if default font has been loaded
    if (raylib_getfontdefault().texture.id != 0)
    {
        ...
        raylib_draw_text_ex(raylib_getfontdefault(), text, position, (float)fontSize, (float)spacing, color);
    }
}

// Draw text using raylib_font
// NOTE: chars spacing is NOT proportional to fontSize
void raylib_draw_text_ex(raylib_font font, const char *text, raylib_vector2 position, float fontSize, float spacing, raylib_color tint)
{
    if (font.texture.id == 0) font = raylib_getfontdefault();  // Security check in case of not valid font

    int size = raylib_textlength(text);    // Total size in bytes of the text, scanned by codepoints in loop
    ...
    for (int i = 0; i < size;)
    {
        ...
        int codepoint = raylib_getcodepointnext(&text[i], &codepointByteCount);
        int index = raylib_getglyphindex(font, codepoint);

        if (codepoint == '\n')
        {
            // NOTE: Line spacing is a global variable, use raylib_settextlinespacing() to setup
        ...
        else
        {
            ...
            {
                raylib_drawtextcodepoint(font, codepoint, (raylib_vector2){ position.x + textOffsetX, position.y + textOffsetY }, fontSize, tint);
            }
...
// Draw text using raylib_font and pro parameters (rotation)
void raylib_drawtextpro(raylib_font font, const char *text, raylib_vector2 position, raylib_vector2 origin, float rotation, float fontSize, float spacing, raylib_color tint)
{
    rlPushMatrix();
        ...
        raylib_draw_text_ex(font, text, (raylib_vector2){ 0.0f, 0.0f }, fontSize, spacing, tint);
...
void raylib_drawtextcodepoint(raylib_font font, int codepoint, raylib_vector2 position, float fontSize, raylib_color tint)
{
    ...
    raylib_drawtexturepro(font.texture, srcRec, dstRec, (raylib_vector2){ 0, 0 }, 0.0f, tint);
}
...
void raylib_drawtextcodepoints(raylib_font font, const int *codepoints, int codepointCount, raylib_vector2 position, float fontSize, float spacing, raylib_color tint)
{
    ...
    for (int i = 0; i < codepointCount; i++)
    {
        int index = raylib_getglyphindex(font, codepoints[i]);
        ...
        else
        {
            ...
            {
                raylib_drawtextcodepoint(font, codepoints[i], (raylib_vector2){ position.x + textOffsetX, position.y + textOffsetY }, fontSize, tint);
            }
...
{
    raylib_vector2 textSize = { 0.0f, 0.0f };
    ...
    if (raylib_getfontdefault().texture.id != 0)
    {
        ...
        textSize = raylib_measuretextex(raylib_getfontdefault(), text, (float)fontSize, (float)spacing);
    }

    return (int)textSize.x;
}

// Measure string size for raylib_font
raylib_vector2 raylib_measuretextex(raylib_font font, const char *text, float fontSize, float spacing)
{
    raylib_vector2 textSize = { 0 };

    if ((font.texture.id == 0) || (text == NULL)) return textSize; // Security check

    int size = raylib_textlength(text);    // Get size in bytes of text
    ...
    for (int i = 0; i < size;)
    {
        ...
        int next = 0;
        letter = raylib_getcodepointnext(&text[i], &next);
        index = raylib_getglyphindex(font, letter);
...
{
    raylib_glyphinfo info = { 0 };

    info = font.glyphs[raylib_getglyphindex(font, codepoint)];
...
raylib_rectangle raylib_getglyphatlasrec(raylib_font font, int codepoint)
{
    ...
    rec = font.recs[raylib_getglyphindex(font, codepoint)];
...
// REQUIRES: strcmp()
bool raylib_textisequal(const char *text1, const char *text2)
{
    bool result = false;
...
char *raylib_textreplace(const char *text, const char *replace, const char *by)
{
    ...
    replaceLen = raylib_textlength(replace);
    if (replaceLen == 0) return NULL;  // Empty replace causes infinite loop during count

    byLen = raylib_textlength(by);
    ...
    temp = result = (char *)RL_MALLOC(raylib_textlength(text) + (byLen - replaceLen)*count + 1);
...
{
    int textLen = raylib_textlength(text);
    int insertLen = raylib_textlength(insert);
...
const char *raylib_textjoin(const char **textList, int count, const char *delimiter)
{
    ...
    int totalLength = 0;
    int delimiterLen = raylib_textlength(delimiter);

    for (int i = 0; i < count; i++)
    {
        int textLength = raylib_textlength(textList[i]);
...
void raylib_textappend(char *text, const char *append, int *position)
{
    ...
    *position += raylib_textlength(append);
}
...
char *raylib_loadutf8(const int *codepoints, int length)
{
    ...
    {
        utf8 = raylib_codepointtoutf8(codepoints[i], &bytes);
...
int *raylib_loadcodepoints(const char *text, int *count)
{
    ...
    {
        codepoints[codepointCount] = raylib_getcodepointnext(text + i, &codepointSize);
        i += codepointSize;
    }
...
int raylib_getcodepointcount(const char *text)
{
    ...
    {
        int next = 0;
        raylib_getcodepointnext(ptr, &next);
...
int raylib_getcodepointprevious(const char *text, int *codepointSize)
{
    ...
    *codepointSize = 0;
    ...
    codepoint = raylib_getcodepointnext(ptr, &cpSize);
...
static raylib_font LoadBMFont(const char *fileName)
{
    ...
    if (readVars < 4) { raylib_unloadfiletext(fileText); return font; } // Some data not available, file malformed

    if (pageCount > MAX_FONT_IMAGE_PAGES)
    {
        TRACELOG(LOG_WARNING, "FONT: [%s] raylib_font defines more pages than supported: %i/%i", fileName, pageCount, MAX_FONT_IMAGE_PAGES);
    ...
    for (int i = 0; i < pageCount; i++)
    {
        ...
        if (readVars < 1) { raylib_unloadfiletext(fileText); return font; } // No fileName read
    }
    ...
    if (readVars < 1) { raylib_unloadfiletext(fileText); return font; } // No glyphCount read

    // Load all required images for further compose
    raylib_image *imFonts = (raylib_image *)RL_CALLOC(pageCount, sizeof(raylib_image)); // raylib_font atlases, multiple images

    for (int i = 0; i < pageCount; i++)
    {
        imFonts[i] = raylib_load_image(raylib_textformat("%s/%s", raylib_getdirectorypath(fileName), imFileName[i]));

        if (imFonts[i].format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE)
        {
            // Convert image to GRAYSCALE + ALPHA, using the mask as the alpha channel
            raylib_image imFontAlpha = {
            ...
            raylib_unloadimage(imFonts[i]);
            imFonts[i] = imFontAlpha;
        }
    }

    raylib_image fullFont = imFonts[0];
    for (int i = 1; i < pageCount; i++) raylib_unloadimage(imFonts[i]);
    ...
    if (pageCount > 1)
    {
        ...
        raylib_imageresizecanvas(&fullFont, imWidth, imHeight*pageCount, 0, 0, raylib_black);
        ...
        for (int i = 1; i < pageCount; i++)
        {
            ...
            raylib_imagedraw(&fullFont, imFonts[i], srcRec, destRec, raylib_white);
        }
    }

    RL_FREE(imFonts);

    font.texture = raylib_loadtexturefromimage(fullFont);

    // Fill font characters info data
    font.baseSize = fontSize;
    font.glyphCount = glyphCount;
    font.glyphPadding = 0;
    font.glyphs = (raylib_glyphinfo *)RL_MALLOC(glyphCount*sizeof(raylib_glyphinfo));
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        if (readVars == 9)  // Make sure all char data has been properly read
        {
            ...
            font.glyphs[i].image = raylib_imagefromimage(fullFont, font.recs[i]);
        }
        else
        {
            font.glyphs[i].image = raylib_genimagecolor(font.recs[i].width, font.recs[i].height, raylib_black);
            TRACELOG(LOG_WARNING, "FONT: [%s] Some characters data not correctly provided", fileName);
        }
    }

    raylib_unloadimage(fullFont);
    raylib_unloadfiletext(fileText);

    if (font.texture.id == 0)
    {
        raylib_unloadfont(font);
        font = raylib_getfontdefault();
        TRACELOG(LOG_WARNING, "FONT: [%s] Failed to load texture, reverted to default font", fileName);
    }
    else TRACELOG(LOG_INFO, "FONT: [%s] raylib_font loaded successfully (%i glyphs)", fileName, font.glyphCount);
...
static raylib_glyphinfo *LoadFontDataBDF(const unsigned char *fileData, int dataSize, int *codepoints, int codepointCount, int *outFontSize)
{
    ...
    int fontBBw = 0;                // raylib_font base character bounding box width
    int fontBBh = 0;                // raylib_font base character bounding box height
    int fontBBxoff0 = 0;            // raylib_font base character bounding box X0 offset
    int fontBByoff0 = 0;            // raylib_font base character bounding box Y0 offset
    int fontAscent = 0;             // raylib_font ascent
    ...
    raylib_glyphinfo *charGlyphInfo = NULL; // Pointer to output glyph info (NULL if not set)
    ...
    glyphs = (raylib_glyphinfo *)RL_CALLOC(codepointCount, sizeof(raylib_glyphinfo));
