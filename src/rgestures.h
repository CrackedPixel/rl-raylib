...
// raylib_gesture event
typedef struct {
    int touchAction;
    int pointCount;
    int pointId[MAX_TOUCH_POINTS];
    raylib_vector2 position[MAX_TOUCH_POINTS];
} GestureEvent;
...
#if defined(RGESTURES_STANDALONE)
void raylib_setgesturesenabled(unsigned int flags);            // Enable a set of gestures using flags
bool raylib_isgesturedetected(int gesture);                    // Check if a gesture have been detected
int raylib_getgesturedetected(void);                           // Get latest detected gesture

float raylib_getgestureholdduration(void);                     // Get gesture hold time in seconds
raylib_vector2 raylib_getgesturedragvector(void);                     // Get gesture drag vector
float raylib_getgesturedragangle(void);                        // Get gesture drag angle
raylib_vector2 raylib_getgesturepinchvector(void);                    // Get gesture pinch delta
float raylib_getgesturepinchangle(void);                       // Get gesture pinch angle
#endif
...
typedef struct {
    ...
    struct {
        ...
        raylib_vector2 upPosition;             // Touch up position
        raylib_vector2 downPositionA;          // First touch down position
        raylib_vector2 downPositionB;          // Second touch down position
        raylib_vector2 downDragPosition;       // Touch drag position
        raylib_vector2 moveDownPositionA;      // First touch down position on move
        raylib_vector2 moveDownPositionB;      // Second touch down position on move
        raylib_vector2 previousPositionA;      // Previous position A to compare for pinch gestures
        raylib_vector2 previousPositionB;      // Previous position B to compare for pinch gestures
    ...
    struct {
        raylib_vector2 vector;                 // DRAG vector (between initial and current position)
    ...
    struct {
        raylib_vector2 vector;                 // PINCH vector (between first and second touch points)
...
//----------------------------------------------------------------------------------
static float rgVector2Angle(raylib_vector2 initialPosition, raylib_vector2 finalPosition);
static float rgVector2Distance(raylib_vector2 v1, raylib_vector2 v2);
...
bool raylib_isgesturedetected(unsigned int gesture)
{
...
void ProcessGestureEvent(GestureEvent event)
{
    ...
    if (GESTURES.Touch.pointCount == 1)     // One touch point
    {
        if (event.touchAction == TOUCH_ACTION_DOWN)
        {
            ...
            GESTURES.Drag.vector = (raylib_vector2){ 0.0f, 0.0f };
        }
        else if (event.touchAction == TOUCH_ACTION_UP)
        {
            ...
            GESTURES.Touch.downDragPosition = (raylib_vector2){ 0.0f, 0.0f };
    ...
    else if (GESTURES.Touch.pointCount == 2)    // Two touch points
    {
        ...
        else if (event.touchAction == TOUCH_ACTION_UP)
        {
            ...
            GESTURES.Pinch.vector = (raylib_vector2){ 0.0f, 0.0f };
            GESTURES.Touch.pointCount = 0;
...
static float rgVector2Angle(raylib_vector2 v1, raylib_vector2 v2)
{
...
// Calculate distance between two raylib_vector2
static float rgVector2Distance(raylib_vector2 v1, raylib_vector2 v2)
{
    float result;
