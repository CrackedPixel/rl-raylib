...
#include <jni.h>                        // Required for: JNIEnv and JavaVM [Used in raylib_openurl()]
...
static const raylib_keyboardkey KeycodeMap[KEYCODE_MAP_SIZE] = {
...
static raylib_gamepadbutton AndroidTranslateGamepadButton(int button);                     // Map Android gamepad button to raylib gamepad button
...
static void AndroidCommandCallback(struct android_app *app, int32_t cmd)
{
    switch (cmd)
    {
                ...
                else
                {
                    ...
                    if (assetsReloadRequired)
                    {
                        for (int i = 0; i < assetCount; i++)
                        {
                            ...
                            (*textureAsset + i) = raylib_loadtexture(assetPath[i]);
                        }
                    }
                    */
                }
        ...
        case APP_CMD_TERM_WINDOW:
        {
            ...
            // this means that the user has already called 'raylib_closewindow()'
...
static raylib_gamepadbutton AndroidTranslateGamepadButton(int button)
{
    switch (button)
    {
...
static int32_t AndroidInputCallback(struct android_app *app, AInputEvent *event)
{
    ...
    else if (type == AINPUT_EVENT_TYPE_KEY)
    {
            ...
            ((source & AINPUT_SOURCE_GAMEPAD) == AINPUT_SOURCE_GAMEPAD))
        {
            ...
            raylib_gamepadbutton button = AndroidTranslateGamepadButton(keycode);
        ...
        raylib_keyboardkey key = (keycode > 0 && keycode < KEYCODE_MAP_SIZE) ? KeycodeMap[keycode] : KEY_NULL;
        if (key != KEY_NULL)
        {
    ...
    for (int i = 0; (i < CORE.Input.Touch.pointCount) && (i < MAX_TOUCH_POINTS); i++)
    {
        ...
        CORE.Input.Touch.position[i] = (raylib_vector2){ AMotionEvent_getX(event, i), AMotionEvent_getY(event, i) };
